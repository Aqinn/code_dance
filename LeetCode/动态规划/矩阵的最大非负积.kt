...
fun maxProductPath(grid: Array<IntArray>): Int {
    ...
    val maxDp = Array(grid.size) { LongArray(grid[0].size) }
    val minDp = Array(grid.size) { LongArray(grid[0].size) }
    for (i in grid.indices) {
        for (j in grid[i].indices) {
            if (i == 0 && j == 0) {
                maxDp[i][j] = grid[i][j].toLong()
                minDp[i][j] = grid[i][j].toLong()
            } else if (i == 0 && j != 0) {
    ...
    return if (maxDp[grid.size - 1][grid[grid.size - 1].size - 1] < 0) {
        -1
    } else {
        (maxDp[grid.size - 1][grid[grid.size - 1].size - 1] % 1000000007).toInt()
    }
}